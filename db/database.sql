...
CREATE TABLE IF NOT EXISTS `linktree`.`photo_users` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `id_photo_user` VARCHAR(200) NULL,
...
CREATE TABLE IF NOT EXISTS `linktree`.`profile` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `bio` VARCHAR(80) NULL,
  `profile_title` VARCHAR(45) NULL,
  `users_id1` INT NOT NULL,
  PRIMARY KEY (`id`, `users_id1`),
  INDEX `fk_profile_users1_idx` (`users_id1` ASC) VISIBLE,
...
DELIMITER $$
USE `linktree`$$
CREATE DEFINER = CURRENT_USER TRIGGER `linktree`.`users_AFTER_INSERT_photo_users` AFTER INSERT ON `users` FOR EACH ROW
BEGIN
	INSERT INTO photo_users (users_id) VALUES (new.id);
END$$

USE `linktree`$$
CREATE DEFINER = CURRENT_USER TRIGGER `linktree`.`users_AFTER_INSERT_profile` AFTER INSERT ON `users` FOR EACH ROW
BEGIN
	INSERT INTO profile (users_id) VALUES (new.id);
END$$

USE `linktree`$$
CREATE DEFINER = CURRENT_USER TRIGGER `linktree`.`users_AFTER_INSERT_security_users` AFTER INSERT ON `users` FOR EACH ROW
BEGIN
